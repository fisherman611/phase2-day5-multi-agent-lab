...
Lệnh này chạy single-agent baseline bằng `LLMClient` hiện tại.
...
Lệnh này chạy workflow supervisor -> researcher -> analyst -> writer và trả về toàn bộ `ResearchState`.
...
Report markdown sẽ được lưu trong thư mục `reports/`.
...
## Checklist mở rộng cho học viên

Bạn có thể mở rộng thêm theo hướng production:

1. Nâng cấp prompt quality và output schema theo domain cụ thể.
2. Kết nối search provider thật (Tavily/Bing/SerpAPI) và thêm ranking.
3. Tinh chỉnh routing policy bằng confidence/quality signals.
4. Thêm critic loop tự động rewrite khi chất lượng thấp.
5. Kết nối tracing provider thật: LangSmith, Langfuse hoặc OpenTelemetry.
6. Mở rộng benchmark dataset và rubric chấm điểm.
